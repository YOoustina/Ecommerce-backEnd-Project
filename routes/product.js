...
router.post('/add', authnticationMiddleware, async (req, res, next) => {
  try {
    const { discount, price, imagesUrls, data, categoryId } = req.body;
    ...
    res
      .status(200)
      .json(
        ` ${product.data[0].name} added with ${product.data[0].description} and ${product}`
      );
  } catch (err) {
    next(err);
  }
...
router.get('/', async (req, res, next) => {
  const { limit, skip, sortBy, sdir } = req.query;
  ...
  const product =
    limit & sortBy
      ? await Products.find()
          ...
          .sort(sortObject)
      : sortBy
      ? await Products.find().sort(sortObject)
      : limit
      ? await Products.find()
          .limit(+limit)
          .skip(+skip)
